...
const EditModal: React.FC<EditModalProps> = ({ row, isOpen, handleModal, actionRef }) => {
    ...
    const { id, type } = history.location.query as { id: string, type: string }
    ...
    const handleAdd = async (params: any) => {
        const res = await reqAdd({ ...params, type, parentId: id })
        if (res?.code === 200) {
    ...
    const handleEdit = async (params: any) => {
        const res = await reqEdit({ ...params, type, parentId: id, id: row?.id })
        if (res?.code === 200) {
            message.success(res?.message)
            actionRef?.current.reload()
            handleModal(false)
